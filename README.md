...
**新项目文档已按主题分类整理到 `docs/` 目录，使用统一序号前缀便于导航：**

- [`docs/01-overview/`](docs/01-overview/) — 项目介绍、PRD、快速开始
- [`docs/02-architecture/`](docs/02-architecture/) — 架构设计、组件说明、管道契约
- [`docs/03-planning/`](docs/03-planning/) — 规划阶段设计、用户旅程、共识模型
- [`docs/04-agents/`](docs/04-agents/) — Agent 设计、角色、实现指南
- [`docs/05-acceptance/`](docs/05-acceptance/) — 验收标准、测试矩阵、质量文档
- [`docs/06-playgrounds/`](docs/06-playgrounds/) — 试验场脚本、示例项目
- [`docs/07-worklog/`](docs/07-worklog/) — 工作日志、开发记录
- [`docs/08-milestones/`](docs/08-milestones/) — 里程碑、路线图、DoD
- [`docs/README.md`](docs/README.md) — **统一文档入口**（推荐首先阅读）
...
```bash
# 仅预览（不修改文件）
bash scripts/organize_docs_preview.sh
